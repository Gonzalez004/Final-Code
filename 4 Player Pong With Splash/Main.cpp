...
int main(int argc, char **argv)
{
	...
	FootSteps = al_load_sample("DinoFootstep.wav");
	Roar = al_load_sample("DinoRoar.wav");

	SongInstance = al_create_sample_instance(Roar);
	...
	ALLEGRO_BITMAP *Photo1 = al_load_bitmap("Dino.jpg");

	al_set_window_position(display, 0, 0); 
	al_set_window_title(display, "Ferny's Game");
	...
	al_convert_mask_to_alpha(Photo1, al_map_rgb(0, 0, 0));

	al_play_sample(FootSteps, 1, 0, 1, ALLEGRO_PLAYMODE_ONCE, 0);
	al_rest(2);
	al_play_sample(Roar, 1, 0, 1, ALLEGRO_PLAYMODE_ONCE, 0);
	for (int i = 0; i < 10; i++) {
		al_draw_tinted_bitmap(Photo1, al_map_rgba_f(0.1*i, 0.1*i, 0.1*i, 0.5), 550, 50, 0);
		al_flip_display();
