...
enum directions { RIGHT, LEFT, UP, DOWN, NOTHING };
...
int main() {
	...
	bool redraw = true;
	bool doexit = false;
	float bouncer_dx = -4.0, bouncer_dy = 5.0;
	//set up Allegro
	al_init();
	al_init_image_addon();
	al_install_keyboard();
	...
	while (!doexit)
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if (key[KEY_UP] && player_y >= 0) {
				dir = UP;
				player_y -= 4.0;

			}

			if (key[KEY_DOWN] && player_y <= 800-32) {
				dir = DOWN;
				player_y += 4.0;
			...
			if (key[KEY_RIGHT] && player_x <= 800-32) {
				dir = RIGHT;
				player_x += 4.0;
		...
		if (redraw && al_is_event_queue_empty(event_queue)) {
			...
			al_clear_to_color(al_map_rgb(255, 255, 255));
			if (key[KEY_UP] == false && key[KEY_DOWN] == false && key[KEY_LEFT] == false && key[KEY_RIGHT] == false) {
				int dir = NOTHING;
				///////////////////////////////////////////////////////////////////////////////////////////////////
				frameCount++; //tick the delay counter
				if (frameCount >= frameDelay) { //has enough time passed?
				...
				///////////////////////////////////////////////////////////////////////////////////////////////////
				if (player_x < 0 || player_x > 800 - 32) {
					bouncer_dx = -bouncer_dx;
				}
				//if the box hits the top wall OR the bottom wall
				if (player_y < 0 || player_y > 800 - 32) {
